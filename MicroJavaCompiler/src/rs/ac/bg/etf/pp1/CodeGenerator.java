...
boolean assignOp=false,mulOp=false, divOp=false, subOp=false, modOp=false, addOp=true;
boolean newVisit=false,element=false;
boolean arrayLeft=false;
boolean addopBefore=true;
boolean incOp=false, decOp=false;
boolean termMinus=false,firstArray=false;//sluzi za nivo ugnjezdenosti niza u nizu.. ako je true znaci da smo u prvom nizu npr niz[.. nalazimo se u niz
                                         //sluzi za terMinus i za read
int openCnt=0;                         
...
Obj designObj=Tab.noObj;
	...
	public void visit(PrintStatement print) {
		...
		if (print.getExpr().struct.getKind()==Struct.Int) {
		...
		else if(print.getExpr().struct.getKind()==Struct.Array) {
			...
			else if (print.getExpr().struct.getElemType().getKind()==Struct.Bool){
...
public void visit (NumberFactor num) {
	...
	Obj con=Tab.insert(Obj.Con,"Dule car",num.struct);
	...
	if (termMinus && !firstArray) {Code.put(Code.neg);termMinus=false;}
...
public void visit(CharFactor factor) {
	...
	Obj con=Tab.insert(Obj.Con, "Dule car", factor.struct);
	con.setLevel(0);
...
public void visit(BoolFactor bool) {
	...
	Obj con=Tab.insert(Obj.Con,"Dule car",bool.struct);
...
public void visit(Designator designator) {
	 ...
	 if (DesignatorStatement.class!=parent.getClass() && designator.obj.getType().getKind()!=Struct.Array && FuncCall.class!=parent.getClass() && ProcCall.class!=parent.getClass()) {
		 Code.load(designator.obj);
		 
		 if (termMinus && !firstArray) {Code.put(Code.neg); //ako je usao u TermMinus tj MINUS od Term Minus
	...
	//	 Code.load(designator.obj);
	 ...
	 if (element && !readVisited) {
		 ...
		 if (DesignatorStatement.class!=parent.getClass()) {//ako nije sa leve strane =
		 ...
		 if (termMinus && !firstArray) {Code.put(Code.neg); //ako je usao u TermMinus tj MINUS od Term Minus
			...
			termMinus=false;}
			 ...
			 }  //element niza u expr stavi mu vrednos na stek samo na stek
		 
		 else arrayLeft=true; //elemnt niza je sa leve strane zapamti tu sitiaciju kad stignes i designstatemen
...
Obj incHelper=new Obj(Obj.Var,"Dule Car", Tab.intType);
...
Obj con=new Obj(Obj.Con,"Dule Car",new Struct(Struct.Int));
...
public void visit(NewFact newFact) {
		...
		if (newFact.getNewType().getType().struct.getKind()==Struct.Int)
	Code.put(1);
...
if(openCnt==0 && (termMinus||readVisited)) firstArray=false; //kada se poslednja zagrda dok je termMinus zatvori znaci stizemo da ciljanog niza
...
public void visit(ReadStatement readStmt) {
	...
	if (designObj.getType().getKind()==Struct.Int || designObj.getType().getKind()==Struct.Bool)
	Code.put(Code.read);
	
	else if (designObj.getType().getKind()==Struct.Char) Code.put(Code.bread);
	
	else if(designObj.getType().getKind()==Struct.Array )
	{
		
		if (designObj.getType().getElemType().getKind()==Struct.Int || designObj.getType().getElemType().getKind()==Struct.Bool)
			Code.put(Code.read);
			
			else if (designObj.getType().getElemType().getKind()==Struct.Char) Code.put(Code.bread);
